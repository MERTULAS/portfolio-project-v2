...
export const metadata: Metadata = {
  ...
  description: 'Mert Ulas, Frontend Developer and Software Engineer. Learn about my experience with Next.js, React.js, Python, JavaScript and more.',
  keywords: 'mert ulaş, yazılım mühendisi, frontend developer, react, nextjs, javascript, typescript, web geliştirme, portfolio, deneyim, yazılım projeleri, experience, cv, about, Mert Ulaş, Mert Ulas, Mert Ulas CV, Mert Ulas Profil, Mert Ulas Portfolio, Mert Ulas About',
  alternates: {
    canonical: 'https://mertulas.dev/about',
  },
  robots: {
    index: true,
    follow: true,
  ...
  twitter: {
    card: 'summary',
    title: 'Mert Ulas | Software Developer',
    description: 'Personal portfolio and CV page of Mert Ulas who works as a Frontend Developer',
...
const AboutPage = () => {
  ...
  const personSchema = {
    ...
    "sameAs": [
      ...
      "https://x.com/mertulaas"
    ],
    "workExperience": character.working_history.map((job: any) => ({
      "@type": "Organization",
      "name": job.company,
    ...
    "knowsAbout": character.skills.weapons.map((skill: any) => skill.name),
  ...
  return (
    <div>
      ...
      <main className="page about-page">
        <div className="cyberpunk__glitch-overlay" aria-hidden="true"></div>
        <div className="cyberpunk__container">
          <section className="cyberpunk__profile" aria-labelledby="profile-heading">
            <div className="profile-header">
              <div className="profile-header__level">
                <div className="level-ring" aria-hidden="true"></div>
                <div className="level-text" aria-hidden="true">
                  {/* character.level() */}
                </div>
                <Image src={'/icons/apple-touch-icon.png'} alt="Mert Ulaş profil resmi" width={80} height={80} />
              </div>
              <div className="profile-header__title">
                <div className="profile-header__title-text">
            ...
            <div className="neo-panel profile-info">
              <h2 className="neo-panel__title" id="about-me-heading">
                <span className="neo-panel__icon" aria-hidden="true">⟁</span>
                AGENT DATA
              </h2>
              <div className="profile-info__content">
                {stringParser(character.info).map((text: string, index: number) => (
                  <p key={index} className="profile-info__text">{text}</p>
                ))}
              </div>
            </div>
          </section>

          <section className="cyberpunk__content">
            <div className="neo-panel skills-panel">
              <h2 className="neo-panel__title" id="skills-heading">
                <span className="neo-panel__icon" aria-hidden="true">⟰</span>
                TECH SKILLS
              </h2>
            ...
            <div className="neo-panel mission-panel">
              <h2 className="neo-panel__title" id="experience-heading">
                <span className="neo-panel__icon" aria-hidden="true">⟲</span>
                MISSION LOG
              </h2>
              <div className="mission-timeline">
                {character.working_history.map((job: any, index: number) => (
                  <div key={index} className="mission-card">
                    <div className="mission-card__header">
                      <div className="mission-card__title">
                        <h3>{job.position}</h3>
                        <span className="mission-card__company">{job.company}</span>
                      </div>
                      <span className="mission-card__period">{job.period}</span>
                    </div>
                    <p className="mission-card__desc">{job.description}</p>
                  </div>
                ))}
              </div>
            </div>
